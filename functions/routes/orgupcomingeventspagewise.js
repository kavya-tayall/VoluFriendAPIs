...
// Endpoint to get paginated events and their shifts
router.get("/", async (req, res) => {
  const { org_user_id, start_date, end_date, page = 1, pageSize = 10 } = req.query;
  ...
  try {
    ...
    // Apply pagination
    const paginatedEventIds = validEventIds.slice((page - 1) * pageSize, page * pageSize);

    if (paginatedEventIds.length === 0) {
      return res.status(200).json({ message: "No events found for the given criteria" });
    }

    // Fetch shifts for events in parallel and calculate totals
    const shiftsForEvent = await getShiftsForEvents(paginatedEventIds);

    // Prepare event data map
    const eventMap = {};

    await Promise.all(paginatedEventIds.map(async (event_id) => {
    ...
    const totalPages = Math.ceil(validEventIds.length / pageSize);
    const pagination = {
      page: Number(page),
      pageSize: Number(pageSize),
      totalPages,
      totalEvents: validEventIds.length
    };

    // Return paginated events and shifts data
    res.json({ events: eventMap, pagination });
  } catch (error) {
